...
export const batchAddItemsSchema = z.object({
  ...
  ).min(1, 'At least one item required').max(100, 'Cannot add more than 100 items at once'),
});
...
export const batchRemoveItemsSchema = z.object({
  ...
  (data: { itemIds?: string[]; entityIds?: (string | number)[] }) => data.itemIds || data.entityIds,
